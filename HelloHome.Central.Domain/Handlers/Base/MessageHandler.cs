...
namespace HelloHome.Central.Domain.Handlers.Base
{
    ...
    {
        Task<IList<OutgoingMessage>> HandleAsync(Message request, CancellationToken cToken);
    }
    ...
    public abstract class MessageHandler<T> : IMessageHandler where T : IncomingMessage
    {
        ...
        public async Task<IList<OutgoingMessage>> HandleAsync(Message request, CancellationToken cToken)
        {
            if (request.GetType() != typeof(T))
                throw new ArgumentException(
                    $"request of type {request.GetType().Name} cannot be processed by {this.GetType().Name}");
